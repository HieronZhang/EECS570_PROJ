...
void __global__ reflected_distance(const float *point_x, const float *point_y, const float *point_z, const float *rx_x,
								   const float *rx_y, const float *rx_data, const float *dist_tx,
								   float *image, const int scan_num, int*mutex);
...
int main(int argc, char **argv)
{
	...
	const int pts_r = 1560; // Radial points along scanline
	const int sls_t = size; // Number of scanlines in theta
	const int sls_p = size; // Number of scanlines in phi
	const int scan_num = pts_r * sls_t * sls_p;
	...
	int grid_size = data_num / block_size;
	...
	//TODO: MUTEX
	int *mutex;
	cudaMalloc((void **)&mutex, scan_num * sizeof(int));// each pixel has one mutex num, so access to image[i] should acquire mutex[i]
	cudaMemset(mutex, 0, scan_num * sizeof(int));

	transmit_distance<<<grid_size, block_size>>>(d_point_x, d_point_y, d_point_z, d_dist_tx);

	const int trans_num = TRANS_X * TRANS_Y;
	grid_size = trans_num / block_size;
	reflected_distance<<<grid_size, block_size>>>(d_point_x, d_point_y, d_point_z, 
												d_rx_x, d_rx_y, d_rx_data, d_dist_tx, d_image, scan_num, mutex);
...
void __global__ reflected_distance(const float *point_x, const float *point_y, const float *point_z,
								   const float *rx_x, const float *rx_y, const float *rx_data, const float *dist_tx,
								   float *image, const int scan_num, int *mutex)
{
	const int it_rx = blockDim.x * blockIdx.x + threadIdx.x;
	const int offset = it_rx * DATA_LEN;
	...
	for (int point = 0; point < scan_num; point++)
	{
		...
		do
		{
			//if (locked = (atomicCAS(mutex + point, 0, 1) == 0))
			if (locked = (atomicCAS(&mutex[0], 0, 1) == 0))
			{
