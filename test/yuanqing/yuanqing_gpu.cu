...
void __global__ transmit_distance(const float *point_x, const float *point_y, const float *point_z, float *dist_tx)
{
	...
	const int point_start = thread_id * PTS_R;
	const int point_end = point_start + PTS_R;
...
void __global__ reflected_distance(const float *point_x, const float *point_y, const float *point_z, 
									const float *rx_x, const float *rx_y, const float *rx_data, const float *dist_tx, float *image)
{
	const int thread_id  = blockDim.x * blockIdx.x + threadIdx.x;
	const int point_start = thread_id * PTS_R;
	const int point_end = point_start + PTS_R;
	...
	float const1, const2, pre_dist;

	for (int it_rx = 0; it_rx < TRANS_X * TRANS_Y; it_rx++)
	{
		point = point_start;
		delta_x = (point_x[point_end - 1] - point_x[point_start]) / (PTS_R - 1);
		delta_y = (point_y[point_end - 1] - point_y[point_start]) / (PTS_R - 1);
		delta_z = (point_z[point_end - 1] - point_z[point_start]) / (PTS_R - 1);
		const1 = delta_x * delta_x + delta_y * delta_y + delta_z * delta_z;
		const2 = -2 * (rx_x[it_rx] * delta_x + rx_y[it_rx] * delta_y);
		pre_dist = 0;
		for (int it_r = 0; it_r < 1; it_r++)
		{
			...
			pre_dist = (float)sqrt(x_comp + y_comp + z_comp);
			dist = dist_tx[point++] + pre_dist;
			index = (int)(dist / IDX_CONST + FILTER_DELAY + 0.5);
			image[point] += rx_data[index + offset];
		}
		for (int it_r = 1; it_r < PTS_R; it_r++)
		{
			pre_dist = (float)sqrt(pre_dist * pre_dist + (2 * PTS_R / 3 + 2 * it_r - 1) * const1 + const2);
			dist = dist_tx[point++] + pre_dist;
			index = (int)(dist / IDX_CONST + FILTER_DELAY + 0.5);
			image[point] += rx_data[index + offset];
		}
