...
int main (int argc, char **argv) {
	...
	for (it_rx = 0; it_rx < trans_x * trans_y; it_rx++) {
		...
		for (it_t = 0; it_t < sls_t; it_t++) {
			for (it_p = 0; it_p < sls_p; it_p++) {
				delta_x = (point_x[point + pts_r - 1] - point_x[point])/(pts_r-1);
				delta_y = (point_y[point + pts_r - 1] - point_y[point])/(pts_r-1);
				delta_z = (point_z[point + pts_r - 1] - point_z[point])/(pts_r-1);
				const1 = delta_x * delta_x + delta_y * delta_y + delta_z * delta_z;
				const2 = -2 * (rx_x[it_rx] * delta_x + rx_y[it_rx] * delta_y);
				pre_dist = 0; 
				for (it_r = 0; it_r < 1; it_r++) {
